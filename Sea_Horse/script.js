window.addEventListener('load', function (params) {
    ...
    class UI {
        ...
        draw(context) {
            ...
            const timeInSec = (this.game.gameTime * 0.001).toFixed(1)
            context.fillText('Timer: ' + timeInSec,20,100)
    ...
    class Game {
        ...
        update(deltaTime) {
            ...
            this.enemies.forEach(enemy => {
                enemy.update();
                if (this.checkHitting(this.player,enemy)) {
                    enemy.markedForDeletion = true
                }
                this.player.projectiles.forEach(projectile => {
                    if (this.checkHitting(projectile,enemy)) {
                        // enemy.markedForDeletion = true
                        enemy.life--;
                        if (enemy.life <= 0) {
                            enemy.markedForDeletion = true
